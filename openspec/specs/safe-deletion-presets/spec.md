# safe-deletion-presets Specification

## Purpose
Defines curated one-click cleanup presets and manual multi-selection. A preset selects only known-safe categories (`SAFE`/`CACHE`) backed by the classifier — never `RISKY`, `DELEGATED`, or `NEVER` — and skips paths absent on the current machine. Users may also manually select multiple files and folders; a mixed selection is planned as a whole with confirmation driven by the highest tier present.
