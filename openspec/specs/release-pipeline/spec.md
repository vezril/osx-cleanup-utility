...
## Purpose
Defines how the project is versioned and released: a `feature/* → development → main` branch model with semantic versioning, where a `vX.Y.Z` tag on `main` triggers a workflow that builds the macOS app and publishes an unsigned `.app`/`.zip` artifact to a GitHub Release. Code signing and notarization are deliberately out of scope at this stage and MUST NOT be required for a release to succeed.
